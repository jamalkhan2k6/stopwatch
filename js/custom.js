  ...
  $(function() {
    ...
    $('.stopwatch').each(function () {
        ...
        function setStopwatch(hours, minutes, seconds, milliseconds) {
            ...
            millisecondsElement.text(prependZero(milliseconds, 2));

            var total_seconds = parseInt(hours) * parseInt(3600) + parseInt(minutes) * parseInt(60) + parseInt(seconds);
            var total_hours = total_seconds / 3600;
            var total_min = total_seconds / 60;
            

            total_hours = total_hours.toFixed(6);

            var count = $("#jcount").val();

             var laps_per_what = $("input[name='laps_speed']:checked").val();

             if(laps_per_what == "hour"){
                $("#count_display").html(parseInt(count/total_hours));
            }else if(laps_per_what == "min"){
                $("#count_display").html(parseInt(count/total_min));
            }else if(laps_per_what == "sec"){
                $("#count_display").html(parseInt(count/total_seconds));
            }
